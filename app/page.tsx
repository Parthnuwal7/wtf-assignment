...
function providerLabel(synthesis: EvidenceSynthesis): string {
  return synthesis.generation.provider === "groq"
    ? `Groq ${synthesis.generation.model.replace(/^openai\//, "").toUpperCase()}`
    : "DeepSeek";
}

function ProcessDetail({ research, synthesis }: { research: PreparedResearchContext; synthesis?: EvidenceSynthesis }) {
  ...
  return <>
    ...
    <ol className="process-list">
      ...
      <li><span>04</span><div><strong>Evidence synthesis</strong><p>{synthesis ? `${providerLabel(synthesis)} generated the answer and was restricted to retrieved source IDs.` : "The synthesis provider was restricted to retrieved source IDs."}</p>{synthesis?.generation.usedFallback && <small>Fallback used: {synthesis.generation.fallbackReason}</small>}</div></li>
    </ol>
  ...
  return <main className="site-shell">
    ...
    {result && <section className="answer-panel" aria-live="polite">
      {result.synthesis ? <>
        ...
        <footer className="answer-footer"><p>Built from {sources.length} retrieved sources · Generated by {providerLabel(result.synthesis)}{result.synthesis.generation.usedFallback ? " fallback" : ""}.</p><div className="answer-actions"><DetailPill icon={<SourceIcon />} label="Sources"><SourcesDetail sources={sources} /></DetailPill><DetailPill icon={<ProcessIcon />} label="Process"><ProcessDetail research={result.research} synthesis={result.synthesis} /></DetailPill></div></footer>
        {result.conversationId && <section className="follow-up-section">
          {followUps.map((turn, index) => <article className="follow-up-answer" key={`${index}-${turn.question}`}>
            ...
            <div className="answer-meta"><span>Follow-up · {providerLabel(turn.synthesis)}</span><span className={`evidence-badge evidence-${turn.synthesis.evidenceLevel}`}>{turn.synthesis.evidenceLevel} evidence</span></div>
